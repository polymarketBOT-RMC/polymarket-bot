...
def research_market(question: str, current_yes_price: float,
    ...
    try:
        ...
        prompt = (
            ...
            f"CRITICAL: TRUE_PROBABILITY must be the probability that {target_side} resolves correctly.\n"
            f"{'This is the same as the YES probability.' if target_side == 'YES' else 'This is 100 minus the YES probability (i.e. probability the event does NOT happen).'}\n"
            f"Need >{min_true_prob:.0f}% probability that {target_side} resolves to recommend BUY.\n\n"
            f"Reply with ONLY these lines:\n"
            f"TRUE_PROBABILITY: [probability {target_side} resolves correctly, 0-100]%\n"
            f"EDGE_PP: [number]\n"
